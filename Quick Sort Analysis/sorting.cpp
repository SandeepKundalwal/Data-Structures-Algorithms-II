...
template<class Item> void mergerArrays(LinearList<Item>& A, int low, int mid, int high){
    int m = mid - low + 1;
    int n = high - mid;
...
template<class Item> void Sort<Item> :: quickSort(LinearList<Item>& A, int low, int high){
    if(low < high){
        int pivot = partitionArray(A, low, high); //finding the pivot index of the array
...
template<class Item> void Sort<Item> :: randomizedQuickSort(LinearList<Item>& A, int low, int high){
    if(low < high){
        int pivot = partitionRandomizer(A, low, high);
        quickSort(A, low, pivot - 1);
        quickSort(A, pivot + 1, high);
    }
}

template<class Item> int partitionArray(LinearList<Item>& A, int low, int high){
...
template<class Item> int partitionRandomizer(LinearList<Item>& A, int low, int high){
    int randomIndex = low + rand() % (high - low);
    swapElements(A, low, randomIndex);
    return partitionArray(A, low, high);
}
